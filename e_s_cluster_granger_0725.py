...
y_emo_data = pd.read_csv('emoji_cluster_score_0723.csv').drop(columns=['Unnamed: 0'])
x_data_temp = []
y_data_temp = []
x_social_dimension = ['Race','Gender','Age','Stature','Wealth ','Health','Ideology','Environment','Science','Food','Housing','Working']
y_emoji_cluster = ['Concerned','Smiling','Negative','Neutral','Tongue','Sleepy','Affection','Unwell']
for item in ['v-']: #,'a-','d-'
    for i in x_social_dimension:
        for j in y_emoji_cluster:
            x_data_temp.append(x_s_d_data[item+i].values)
            y_data_temp.append(y_emo_data[item+j].values)
...
res_idx = []
x_data_temp_t = np.array(x_data_temp, dtype = 'float64')
y_data_temp_t = np.array(y_data_temp, dtype = 'float64')
...
# y_label =[x_social_dimension[i]+' ('+ str(int(item))+')' for i,item in enumerate(granger_every_dimension_sum)]
y_label =x_social_dimension
c = (
    HeatMap(init_opts=opts.InitOpts(width='2000px',height='1000px'))
    .add_xaxis(y_emoji_cluster)
    .add_yaxis(
        "series1",
...
c.render(path='figure_res_0330/heatmap_with_granger_emoji_social_cluster_0725.html')
