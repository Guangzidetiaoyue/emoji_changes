...
all_emojis_adj_scores['emoji'] = emojis
...
for file in all_file_emojis_adj:
    ...
    for e in emojis:
        ...
        emotion_max = max(emojis_emb_score)
        emotion_max_idx = emojis_emb_score.index(emotion_max)
        emotion_max_cls = emotion_class[emotion_max_idx]
        # emoji_emotion_col.append(emotion_max_cls)
        emoji_emotion_col.append(emotion_max_cls+'-'+time)

    all_emojis_adj_scores[time] = emoji_emotion_col

emoji_emotion_df = pd.DataFrame(all_emojis_adj_scores)
# emoji_emotion_df.to_csv('emoji_emotion_score_only_1.csv')
emoji_emotion_df.to_csv('emoji_emotion_score_only_1_time.csv')
print('done')
